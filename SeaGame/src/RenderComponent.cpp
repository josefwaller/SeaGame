		...
		for (b2Fixture* fixture = body->GetFixtureList(); fixture; fixture = fixture->GetNext()) {
			...
			if (b2PolygonShape* shape = dynamic_cast<b2PolygonShape*>(fixture->GetShape())) {
				...
				for (int i = 0; i < shape->m_count; i++) {
					...
					drawableVerts[i] = sf::Vertex({ vert.x * Game::METER_TO_PIXEL, vert.y * Game::METER_TO_PIXEL }, sf::Color::Red);
				}
				...
				drawableShape.setRadius(shape->m_radius * Game::METER_TO_PIXEL);
				b2Vec2 worldPos = body->GetWorldPoint(shape->m_p);
				drawableShape.setPosition({ worldPos.x * Game::METER_TO_PIXEL, worldPos.y * Game::METER_TO_PIXEL });
