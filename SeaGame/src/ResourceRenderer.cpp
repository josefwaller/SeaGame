...
#ifdef _DEBUG
	if (!this->getComponentList().transform) {
...
void ResourceRenderer::reset() {
	...
	GameResource res = std::dynamic_pointer_cast<ResourceController>(this->getComponentList().controller)->getResource();
	this->spr = getResourceSprite(res, true);
	// Set the position
	sf::Vector2f pos = this->getComponentList().transform->getPosition() + sf::Vector2f(32.0f, 32.0f);
